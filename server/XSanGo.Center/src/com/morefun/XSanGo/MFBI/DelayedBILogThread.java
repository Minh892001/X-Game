...
public class DelayedBILogThread extends Thread {
	...
	public void run() {
		...
		while (XsgMFBIManager.getInstance().isStart) {
			try {
				BIMessage BI_Log = msgQueue.poll();
				if (BI_Log != null) {
					Logger.getInstance().log(BI_Log.getUserInfo(),
							BI_Log.getRoleInfo(), BI_Log.getType(),
							BI_Log.getTimestamps(), BI_Log.getParams());
				} else {
	...
	public static void main(String[] args) {
		...
		DelayedBILogThread thread = new DelayedBILogThread();
		thread.start();

		UserInfo userInfo = new UserInfo(true, "testAccount",
				System.currentTimeMillis(), 0, null, null, null, null, "ip",
				null, null, null, null, null, null);

		BIMessage msg = new BIMessage();
		msg.setUserInfo(userInfo);
		msg.setRoleInfo(null);
		msg.setType(EventType.AccountRegister);
		thread.addMess(msg);
