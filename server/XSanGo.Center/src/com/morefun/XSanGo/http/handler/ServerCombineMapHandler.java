 ...
 * 
 */
public class ServerCombineMapHandler extends AbsHandler {
	...
	protected void innerHandle(HttpExchange httpExchange) throws IOException {
		...
		for (GameServerInfo gs : collection) {
			...
			if (orignalId != targetId) {
				list.add(new IntIntPair(orignalId, targetId));
			}
		}

		this.sendResponse(httpExchange, TextUtil.GSON.toJson(list.toArray()));
	}
}
