...
public class LogicThread {
	...
	/** 主逻辑线程 */
	private static ThreadPoolExecutor logicExecutor = new ThreadPoolExecutor(20, 20, 60, TimeUnit.SECONDS,
	...
	private static void loadGameServers() {
		...
		// .getFromApplicationContext(CrossServerMain.getAC());
		// List<GameServer> list = dao.findAll();
		int count = 0;
		for (CrossRangeT r : CrossServerManager.getInstance().crossRangeTs) {
			count += r.serverList.size();
		}
		final CountDownLatch finished = new CountDownLatch(count);
		for (CrossRangeT r : CrossServerManager.getInstance().crossRangeTs) {
			for (Integer id : r.serverList) {
				try {
					// 设置回调
					IceEntry.setupCrossServerCallbackPrx(id, finished);
				} catch (Exception e) {
