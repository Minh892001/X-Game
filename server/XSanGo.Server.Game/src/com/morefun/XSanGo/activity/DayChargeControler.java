...
import com.morefun.XSanGo.db.game.RoleDayCharge;
...
@RedPoint
public class DayChargeControler implements IDayChargeControler, ICharge {
	private IRole rt;
	private Role db;
	...
	public DayChargeControler(IRole rt, Role db) {
		this.rt = rt;
		this.db = db;
	...
	public SummationActivityView getDayChargeView() throws NoteException {
		...
		ActivityT at = XsgActivityManage.getInstance().getActivityT(ActivityTemplate.DayCharge);
	...
	public void receiveDayCharge(int threshold) throws NoteException {
		if (!XsgActivityManage.getInstance().isActivityOpen(ActivityTemplate.DayCharge)) {
		...
		// 记录数据
		RoleDayCharge data = this.db.getDayCharges().get(threshold);
		Date now = Calendar.getInstance().getTime();
		if (data == null) {
			data = new RoleDayCharge(GlobalDataManager.getInstance().generatePrimaryKey(), db, threshold, now);
			this.db.getDayCharges().put(threshold, data);
		} else {
	...
	private DayChargeConsumeT getCurrentActivity(ActivityT at) {
		...
		for (String id : ids) {
			DayChargeConsumeT t = XsgActivityManage.getInstance().dayCharges.get(Integer.parseInt(id));
	...
	public MajorUIRedPointNote getRedPointNote() {
		...
		if (firstOpen && !isOver()) {
			firstOpen = false;
			return new MajorUIRedPointNote(MajorMenu.DayChargeMenu, true);
		}
		...
		for (SummationReward sr : view.configs) {
			if (view.total >= sr.threshold && !isAlreadyReceived(sr.threshold)) {
				return new MajorUIRedPointNote(MajorMenu.DayChargeMenu, false);
			}
		}
		return null;
	}
