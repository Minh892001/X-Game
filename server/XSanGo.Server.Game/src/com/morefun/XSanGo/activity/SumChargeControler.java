...
import com.morefun.XSanGo.db.game.RoleActivityForSumCharge;
...
@RedPoint
public class SumChargeControler implements ISumChargeActivityControler, ICharge {
	private Role db;
	private IRole rt;
	...
	public SumChargeControler(IRole rt, Role db) {
		this.rt = rt;
		this.db = db;
	...
	public SummationActivityView getView() {
		SummationActivityView view = XsgActivityManage.getInstance().cloneSumChargeView();
	...
	private boolean isAlreadyReceived(int threshold) {
		RoleActivityForSumCharge data = this.db.getActivityForSumCharge().get(threshold);
		ActivityT at = getChargeActivityTemplate();
		return data != null
	...
	private ActivityT getChargeActivityTemplate() {
		return XsgActivityManage.getInstance().getActivityT(ActivityTemplate.SumCharge);
	}

	@Override
	public void receiveReward(int threshold) throws NotEnoughYuanBaoException, NoteException {
		if (!XsgActivityManage.getInstance().isActivityOpen(ActivityTemplate.SumCharge)) {
		...
		SummationActivityComponentT sat = XsgActivityManage.getInstance().getSumChargeTemplate(threshold);
		if (sat == null) {
		...
		if (data == null) {
			data = new RoleActivityForSumCharge(GlobalDataManager.getInstance().generatePrimaryKey(), db, threshold,
					now);
			this.db.getActivityForSumCharge().put(threshold, data);
		} else {
	...
	public MajorUIRedPointNote getRedPointNote() {
		...
		if (firstOpen && !isOver()) {
			firstOpen = false;
			return new MajorUIRedPointNote(MajorMenu.SumChargeMenu, true);
		}
		...
		for (SummationReward item : view.configs) {
			if (view.total >= item.threshold && !this.isAlreadyReceived(item.threshold)) {
				return new MajorUIRedPointNote(MajorMenu.SumChargeMenu, false);
			}
		}
