...
import com.morefun.XSanGo.db.game.RoleActivityForSumConsume;
...
@RedPoint
public class SumConsumeControler implements ISumConsumeActivityControler, IYuanbaoChange {
	...
	public SumConsumeControler(IRole rt, Role db) {
		this.rt = rt;
		this.db = db;
	...
	public SummationActivityView getView() {
		...
		ActivityT at = XsgActivityManage.getInstance().getActivityT(ActivityTemplate.SumConsume);
		...
		if (lastTime == null || !DateUtil.isBetween(lastTime, begin, end)) {
			this.db.setConsumeYuanbao(0);
		}
		SummationActivityView view = XsgActivityManage.getInstance().cloneSumConsumeView();
		...
		// 计算累计值
		view.total = this.db.getConsumeYuanbao();
	...
	public void receiveReward(int threshold) throws NoteException, NotEnoughYuanBaoException {
		if (!XsgActivityManage.getInstance().isActivityOpen(ActivityTemplate.SumConsume)) {
		...
		SummationActivityComponentT sat = XsgActivityManage.getInstance().getSumConsumeTemplate(threshold);
		if (sat == null) {
		...
		// 记录数据
		RoleActivityForSumConsume data = this.db.getActivityForSumConsume().get(threshold);
		Date now = Calendar.getInstance().getTime();
		if (data == null) {
			data = new RoleActivityForSumConsume(GlobalDataManager.getInstance().generatePrimaryKey(), db, threshold,
					now);
			this.db.getActivityForSumConsume().put(threshold, data);
		} else {
	...
	private ActivityT getConsumeActivityTemplate() {
		return XsgActivityManage.getInstance().getActivityT(ActivityTemplate.SumConsume);
	}
	...
	public void onYuanbaoChange(int oldBind, int oldUnbind, int newBind, int newUnbind, int changeValue) {
		...
		if (change >= 0// 非消费
				// 不在活动时间内
				|| !XsgActivityManage.getInstance().isActivityOpen(ActivityTemplate.SumConsume)) {
			return;
		}
		...
		// 减去负数来实现增加的目的
		this.db.setConsumeYuanbao(this.db.getConsumeYuanbao() - change);
		// 刷新记录时间
	...
	public MajorUIRedPointNote getRedPointNote() {
		...
		if (firstOpen && !isOver()) {
			firstOpen = false;
			return new MajorUIRedPointNote(MajorMenu.SumConsumeMenu, true);
		}
		...
		for (SummationReward item : view.configs) {
			if (view.total >= item.threshold && !this.isAlreadyReceived(item.threshold)) {
				return new MajorUIRedPointNote(MajorMenu.SumConsumeMenu, false);
			}
		}
