...
public class ExcelParser {
	...
	public static void main(String[] args) {
		List<ActivityT> heroList = parse(ActivityT.class);
	...
	public static <T> List<T> parse(String sheetName, Class<T> type) {
		...
		try {
			ClassPathResource cpr = new ClassPathResource(tableAnnotation.fileName());
			wb = Workbook.getWorkbook(cpr.getFile());
			// wb = Workbook.getWorkbook(new FileInputStream(tableAnnotation
			// .fileName()));

			Sheet sheet = wb.getSheet(sheetName);
			for (int rowIndex = tableAnnotation.beginRow(); rowIndex < sheet.getRows(); rowIndex++) {
				...
				for (Field field : fieldList) {
					ExcelColumn columnAnnotation = field.getAnnotation(ExcelColumn.class);
					ExcelComponet component = field.getAnnotation(ExcelComponet.class);
					Object value = null;
					if (columnAnnotation != null) {
						String cell = getCellContentIgnoreException(cells, columnAnnotation.index());
						value = parseCell(field.getType(), cell, columnAnnotation.dataType());
					} else {
						if (!field.getType().isArray()) {
							throw new NotSupportedException("Only support the array now.");
						}

						Class<?> componentType = field.getType().getComponentType();
						value = Array.newInstance(componentType, component.size());
						...
						for (int i = 0; i < component.size(); i++) {
							...
							for (Field subField : subFields) {
								...
								ExcelColumn subAnno = subField.getAnnotation(ExcelColumn.class);
								if (subAnno != null) {
									int index = component.index() + i * component.columnCount() + subAnno.index();

									subField.set(
											element,
											parseCell(subField.getType(), getCellContentIgnoreException(cells, index),
													subAnno.dataType()));
								}
							}
	...
	private static <T> T parseCell(Class<T> dataType, String cell, DataFormat format) {
		...
		if (dataType.equals(byte.class) || dataType.equals(Byte.class)) {
			...
			value = NumberUtil.parseDouble(cell);
