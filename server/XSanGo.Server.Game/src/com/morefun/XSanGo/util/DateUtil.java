...
public class DateUtil {
	 ...
	 * @return
	 */
	public static int computerDaysBy2Date(long date1, long date2) {
		Calendar start = Calendar.getInstance();
		start.setTimeInMillis(date1);
		Calendar end = Calendar.getInstance();
		end.setTimeInMillis(date2);
		// 判定是同年
		int interverl = start.get(Calendar.YEAR) - end.get(Calendar.YEAR);
		if (interverl == 0) {// 同年直接计算时间差
			return Math.abs(start.get(Calendar.DAY_OF_YEAR) - end.get(Calendar.DAY_OF_YEAR));
		} else {// 不同年，计算跨年
			if (interverl > 0) {
				end.set(Calendar.YEAR, interverl);
				return start.get(Calendar.DAY_OF_YEAR) - end.get(Calendar.DAY_OF_YEAR) + interverl * 365;
			} else {
				start.set(Calendar.YEAR, Math.abs(interverl));
				return end.get(Calendar.DAY_OF_YEAR) - start.get(Calendar.DAY_OF_YEAR) + (-interverl) * 365;
			}
		}
	}
	...
	public static boolean isSameDay(Calendar one, Calendar two) {
		return ((one.get(Calendar.YEAR) == two.get(Calendar.YEAR)) && (one.get(Calendar.DAY_OF_YEAR) == two
	...
	public static boolean isSameMonth(Calendar one, Calendar two) {
		return ((one.get(Calendar.YEAR) == two.get(Calendar.YEAR)) && (one.get(Calendar.MONTH) == two
				.get(Calendar.MONTH)));
	}
	...
	public static boolean isSameWeek(Date date1, Date date2) {
		...
		Calendar two = Calendar.getInstance();
		two.setTime(date2);
		return ((one.get(Calendar.YEAR) == two.get(Calendar.YEAR)) && (one.get(Calendar.WEEK_OF_YEAR) == two
	...
	public static Date joinTime(Date date, Date time) {
		...
		times.setTime(time);
		ca.set(Calendar.HOUR_OF_DAY, times.get(Calendar.HOUR_OF_DAY));
		ca.set(Calendar.MINUTE, times.get(Calendar.MINUTE));
		ca.set(Calendar.SECOND, times.get(Calendar.SECOND));
		ca.set(Calendar.MILLISECOND, times.get(Calendar.MILLISECOND));
		return ca.getTime();
	}

	/**
	 * 对当前日期(年月日)和时间(时分秒)进行合并
	 * 
	 * @param time
	 *            时间（时分秒）
	 * @return
	 */
	public static Date joinTime(Date time) {
		Calendar ca = Calendar.getInstance();
		return joinTime(ca.getTime(), time);
	}
	...
	public static boolean checkTime(Date currDate, Date standardDate) {
		if (DateUtil.compareDate(new Date(), currDate) == 0) {
			if (System.currentTimeMillis() > standardDate.getTime() && currDate.getTime() < standardDate.getTime()) {
				return true;
			} else {
				return false;
			}
	 ...
	 * @return
	 */
	public static boolean checkTimeRange(Date checkTime, Date beginTime, Date endTime) {
	 ...
	 * @return
	 */
	public static boolean checkTimeFormat(String times, String patten) {
		if (TextUtil.isBlank(times) || TextUtil.isBlank(patten)) {
			return false;
		}
		SimpleDateFormat nowDate = new SimpleDateFormat(patten);
		try {
	...
	/**
	 * 验证给定的时间串是否符合指定的时间格式，默认：yyyy-MM-dd HH:mm:ss
	 * 
	 * @param times
	 * @return
	 */
	public static boolean checkTimeFormat(String times) {
		return checkTimeFormat(times, DEFAULT_PATTERN);
	}
}
